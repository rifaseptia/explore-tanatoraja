...
async function getAccommodations(category?: string, page: number = 1, limit: number = 8, search?: string) {
    try {
        await dbConnect();
...
export async function generateMetadata({ params }: Props) {
    ...
    return {
        title: locale === 'id'
            ? 'Penginapan | Explore Tana Toraja'
            : 'Where to Stay | Explore Tana Toraja',
        description: locale === 'id'
            ? 'Temukan penginapan terbaik di Tana Toraja, mulai dari hotel mewah hingga homestay tradisional.'
            : 'Find the best accommodations in Tana Toraja, from luxury hotels to traditional homestays.',
    };
}
...
export default async function StayPage({ params, searchParams }: Props) {
    ...
    const { items: paginatedItems, totalPages } = await getAccommodations(activeCategory, currentPage, limit, searchQuery);
    ...
    const categories = [
        ...
        { id: 'guesthouse', label: locale === 'id' ? 'Penginapan' : 'Guesthouse' },
    ];

    const hero = await getPageHero('stay');
    const heroImage = hero?.image || 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=1920&q=80';
    const heroTitle = hero ? hero.title[locale as 'id' | 'en'] : (locale === 'id' ? 'Tempat Menginap' : 'Where to Stay');
    const heroSubtitle = hero ? hero.subtitle[locale as 'id' | 'en'] : (locale === 'id'
        ? 'Temukan penginapan terbaik untuk petualangan Anda di Tana Toraja'
        : 'Find the perfect accommodation for your journey in Tana Toraja');
            ...
            <div className="max-w-7xl mx-auto px-[38px] relative z-20">
                ...
                <div className="py-12 md:py-16 space-y-5">
                    ...
                    <div className="flex items-center justify-center gap-3">
                        <Suspense fallback={<div className="h-10 w-full max-w-md bg-gray-100 rounded-full animate-pulse" />}>
                            <SearchInput placeholder={locale === 'id' ? 'Cari penginapan' : 'Search accommodation'} />
                        </Suspense>
                    ...
                    <div className="hidden md:flex justify-center gap-2">
                        {categories.map((cat) => (
                            <Link
                                key={cat.id}
                                href={cat.id === 'all'
                                    ? (searchQuery ? `/stay?q=${searchQuery}` : '/stay')
                                    : `/stay?category=${cat.id}${searchQuery ? `&q=${searchQuery}` : ''}`
                                }
                                scroll={false}
                ...
                {paginatedItems.length > 0 ? (
                    ...
                    <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                        ...
                        >
                            {locale === 'id' ? 'Lihat Semua Penginapan' : 'View All Accommodations'}
                        </Link>
                    </div>
                )}
